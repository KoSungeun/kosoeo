package com.study.android.ex54;
...
public class SingerAdapter extends RecyclerView.Adapter<SingerAdapter.SingerItemViewHolder> {
    ...
    public class SingerItemViewHolder extends RecyclerView.ViewHolder {
        ...
        public SingerItemViewHolder(@NonNull View view) {
            super(view);
    ...
    // Adapter의 특정 위치(position)에 있는 데이털르 보여줘야 할때 호출 됩니다.


    @Override
    public void onBindViewHolder(@NonNull SingerItemViewHolder singerItemViewHolder, int i) {
        ArrayList<SingerItem> items2 = items;
        Collections.reverse(items2);

        singerItemViewHolder.textView1.setText(items.get(i).getName());
        singerItemViewHolder.textView2.setText(items.get(i).getAge());
        singerItemViewHolder.imageView1.setImageResource(items.get(i).getResId());

        final int num = i;

        singerItemViewHolder.imageView1.setOnClickListener(new View.OnClickListener() {
            @Override
